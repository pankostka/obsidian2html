...
**Kód je anglicky, texty kolem něj česky.** Názvy funkcí, tříd, konstant a proměnných anglicky. Commit zprávy, `README.cs.md` a `PROJECT.md` česky včetně diakritiky, veřejný `README.md` anglicky. Komentáře a docstringy se teprve překládají, takže jsou zatím české - stav je v `PROJECT.md`.

**Identifikátory nepřejmenovávej hledáním v textu.** Spousta českých slov v kódu nejsou jména, ale výstup: `class="perex"`, `class="karta"`, `hledani.html`, klíče frontmatteru. Přepsat je znamená rozbít `styl.css` ve vaultech, tedy záruku Z40. Použij tokenizér, který rozliší jméno od řetězce, a vynech jména za `args.` - ta vyrobil argparse z přepínačů.
...
**Po každé změně pusť testy**, `python test_md2html.py`. Je jich 43 a běží pod sekundu.
