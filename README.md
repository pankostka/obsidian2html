...
> **This is a translation of [`README.cs.md`](README.cs.md), which is the source.** Changes are made there first and only then carried over here. This file is deleted and made again by translating that one.  
> The exact rules and the reasons behind them are in [SPEC.cs.md](SPEC.cs.md) (Czech); the K and Z codes in brackets point there.

## What it is

This is an **md->html generator**.  
It is a **Python script** that you point at an **input directory of md files** (typically an Obsidian vault, but that is not required), and it writes **HTML files** into an **output directory**.  
The resulting site stands on its own and needs no database.  It is made of **static pages with JavaScript**.
...
| `--edit-links`      | an article gets a link that opens its source in Obsidian (Z90)          |
